    ...
    public class SchedTaskV2Parser
    {
        ...
        public SchedTaskSetting ParseSchedTask(SchedTaskType taskType, XmlNode schedTask)
        {
            ...
            foreach (XmlNode principal in principals)
            {
                ...
                {
                    Id = principal.Attributes?["id"]?.Value,
                    UserId = GetXmlValueSafe(principal, "UserId"),
                    LogonType = GetXmlValueSafe(principal, "LogonType"),
                    RunLevel = GetXmlValueSafe(principal, "RunLevel"),
                    Cpassword = GetXmlValueSafe(principal, "Cpassword")
                };
            ...
            foreach (XmlNode messageAction in messageActions)
            {
                ...
                {
                    Title = GetXmlValueSafe(messageAction, "Title"),
                    Body = GetXmlValueSafe(messageAction, "Body")
                };
            ...
            foreach (XmlNode execAction in execActions)
            {
                ...
                {
                    Command = GetXmlValueSafe(execAction, "Command"),
                    Args = GetXmlValueSafe(execAction, "Arguments"),
                    WorkingDir = GetXmlValueSafe(execAction, "WorkingDirectory")
                };
            ...
            foreach (XmlNode emailAction in emailActions)
            {
                ...
                {
                    From = GetXmlValueSafe(emailAction, "From"),
                    To = GetXmlValueSafe(emailAction, "To"),
                    Subject = GetXmlValueSafe(emailAction, "Subject"),
                    Body = GetXmlValueSafe(emailAction, "Body"),
                    HeaderFields = GetXmlValueSafe(emailAction, "HeaderFields")
                };
